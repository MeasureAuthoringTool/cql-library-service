...
public class CqlDifferentiatorService {
  ...
  /**
   * Compare CQL libraries from two libraries and return normalized, reordered text ready for diff
   * display.
   *
   * @param oldLibraries Map of filename to CQL content for old library
   * @param newLibraries Map of filename to CQL content for new library
   * @param autoReorder Whether to auto-reorder the new library CQL (default true)
  ...
  public List<CqlFileComparisonDTO> compareLibraries(
    ...
    for (Map.Entry<String, String> entry : libraryMap.entrySet()) {
      ...
      if (oldFileName.startsWith("NA-")) {
        // New file without a match in old library
        oldText = "";
      } else {
   ...
   * inconsistencies between different library versions.
   *
  ...
  private Map<String, String> mapByEditDistance(List<String> oldStrings, List<String> newStrings) {
    ...
    for (StringWithMinDist oldStringWithDist : oldStringsWithMinDist) {
      ...
      // If no match found (extra paragraph in old library), map to null
   ...
   * library has more files than old library.
   *
   * @param oldLibraries Old library libraries
   * @param newLibraries New library libraries
  ...
  private Map<String, String> createLibraryMap(
    ...
    // If new library has more files, add unmapped new files with "NA-" prefix
